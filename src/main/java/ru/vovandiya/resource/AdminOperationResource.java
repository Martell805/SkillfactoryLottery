...
public class AdminOperationResource {
    ...
    public Response listAll() {
        return execute(() -> Response.ok(operationService.listAll()).build());
    }

    @GET
    @Path("/{id}")
    public Response getById(@PathParam("id") Long id) {
        return execute(() -> Response.ok(operationService.findById(id)).build());
    }

    @POST
    @Transactional
    public Response create(OperationRequest request) {
        return execute(() -> Response.status(Response.Status.CREATED).entity(operationService.create(request)).build());
    }
    ...
    public Response update(@PathParam("id") Long id, OperationRequest request) {
        return execute(() -> Response.ok(operationService.update(id, request)).build());
    }
